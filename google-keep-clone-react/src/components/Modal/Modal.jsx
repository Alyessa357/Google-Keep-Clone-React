...
const Modal = (props) => {
    const { isModalOpen, selectedNote, toggleModal, editNote, darkMode } = props;
    ...
    return (
        <div className={`modal ${isModalOpen ? "open-modal" : ""}`} onClick={handleCloseModal}>
            <div className="modal-content" onMouseOver={handleMouseOverModal} onMouseOut={handleMouseOutModal} >
                <Form selectedNote={selectedNote} toggleModal={toggleModal} editNote={editNote} darkMode={darkMode} edit />
            </div>
        </div>
    )
}
